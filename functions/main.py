...
def _build_sitemap_xml() -> str:
    ...
    for page in range(1, _SITEMAP_PRODUCT_PAGES + 1):
        ...
        # One retry — confirmed live that Render's backend occasionally
        # 502s transiently (not just cold-start slowness) under back-to-back
        # requests from this loop. A single retry is enough to ride out a
        # one-off blip without turning a flaky page into a truncated sitemap.
        for attempt in range(2):
            try:
                # Render's free-tier backend can cold-start slowly after
                # being idle — an 8s timeout here silently degraded the
                # sitemap down to just the 16 static entries (homepage +
                # brand pages) on the very first deploy, which then cached
                # for an hour with no error surfaced anywhere. 25s
                # comfortably covers a cold start; the print on failure
                # means a real outage is now visible in Cloud Logging
                # instead of silently shipping a stale sitemap.
                resp = requests.get(
                ...
                last_exc = exc
        if items is None:
            print(f"sitemap_xml: /api/products page {page} failed: {last_exc}")
            break
        if not items:
            break
        for item in items:
